...
def fmix(data, targets_A, targets_B, alpha, decay_power=3):
    ...
    if data.dim() == 3:
        # 1D 데이터 (예: (Batch, Channels, Length))
        mask = make_low_freq_mask_1d(decay_power, data.size(2), lam)
        mask = torch.tensor(mask).float().to(data.device)
        mask = mask.unsqueeze(0).unsqueeze(0)  # 배치 및 채널 차원 추가
        mask = mask.expand(data.size(0), data.size(1), data.size(2))
    elif data.dim() == 4:
        # 2D 데이터 (예: (Batch, Channels, Height, Width))
...
def make_low_freq_mask_1d(decay_power, length, lam):
    freqs = np.fft.fftfreq(length) ** 2
    spectrum = np.random.randn(length) * np.exp(-decay_power * freqs)

    mask = np.fft.ifft(spectrum).real
    mask = np.abs(mask)
    mask = (mask - mask.min()) / (mask.max() - mask.min())
    mask = (mask >= np.quantile(mask, 1 - lam)).astype(np.float32)
